...
def main():
    ...
    cs = ctx.cursor()
    try:
        # Lister toutes les tables RAW.YELLOW_TAXI_TRIPS_YYYY_MM
        cs.execute("SHOW TABLES IN SCHEMA RAW;")
        all_tables = cs.fetchall()
        print("Tables présentes dans RAW :")
        for row in all_tables:
            print(row[1])
        tables = [row[1] for row in all_tables if re.match(r'^YELLOW_TAXI_TRIPS_\d{4}_\d{2}$', row[1])]
        print(f"Tables trouvées : {tables}")
        tgt_schema = 'STAGGING'
        cs.execute(f"CREATE SCHEMA IF NOT EXISTS {tgt_schema};")
        for src_table in tables:
            suffix = src_table.replace('YELLOW_TAXI_TRIPS_', '')
            tgt_table = f"CLEAN_TRIPS_{suffix}"
            print(f"Nettoyage et création de {tgt_schema}.{tgt_table} depuis RAW.{src_table} ...")
            sql = f'''
...
'''
            cs.execute(sql)
            print(f"Table créée : {tgt_schema}.{tgt_table}")
    finally:
        cs.close()
        ctx.close()
